...
/** Manejo centralizado de excepciones del microservicio de Juegos. */
...
public class GlobalExceptionHandler {
    ...
    /** 503 - un microservicio remoto no respondio correctamente. */
    @ExceptionHandler(ComunicacionException.class)
    public ResponseEntity<ApiError> manejarComunicacion(ComunicacionException ex) {
        log.error("Error de comunicacion remota: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(new ApiError(HttpStatus.SERVICE_UNAVAILABLE.value(), "Service Unavailable", ex.getMessage()));
    }
